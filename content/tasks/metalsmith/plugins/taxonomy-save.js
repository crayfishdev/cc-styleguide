...
function plugin(options) {
    ...
    collections.forEach(collection => {
        ...
        if(_numOfSubcategories(collection) > 0) {
            // The collection has subcollections that need to be traversed.
            var subcollections = collection.subcollections;

            matcher.subcollections = subcollections.map(subcategory => {
                return ({
    ...
    return function (files, metalsmith, done) {
        ...
        Object.keys(files).forEach( path => {
            ...
            matchers.forEach(matcher => {
                ...
                var subcollection = findSubcollections(matcher, file, path);
                console.log(subcollection);
                collection.pushSubcollection(subcollection);

                /* subcollections nested under a collection. */
                // if(matcher.subcollections && Object.keys(matcher.subcollections).length){
                //     var subcollection;
                //     matcher.subcollections.forEach(submatcher => {
    ...
    function findSubcollections(matcher, file, path) {
        if(!matcher) return;
        var collectionFile = new File({
            title: file.title,
            layout: file.layout,
            filePath: path,
            collections: file.collections
        });
        var collection = new Collection({ label: matcher.key });

        // var collection = getCollection(collections, matchKey);
        
        /** If the matcher obj has a 'match' function, call the 
         * function with the path and file variables as parameters */
        if(typeof matcher.match == 'function') {
            matcher.match(path, file).forEach( isMatch => { 
                // For each sucessful match ... 
                if( isMatch ) {
                    collectionFile.collections = file.collections; // Append the list of matched collections to the file object
                    collection.pushFile(collectionFile); // And add the file to the collection
                }
            });
        }

        if(matcher.subcollections && Object.keys(matcher.subcollections).length) {
            var submatchers = matcher.subcollections;
            submatchers
                .map(submatcher => { return findCollections(submatcher, file, path); })
...
function _numOfSubcategories(collection) {
    if(collection.subcollections) {
        return collection.subcollections.length;
    }
    return 0;
}
